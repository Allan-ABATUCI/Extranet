...
<script>
  ...
  function getTableData() {
    ...
    xhr.open('POST', '?controller=prestataire&action=completer_bdl&annee=<?= $bdl['annee'] ?>&type=<?= $bdl['type_bdl'] ?>&mois=<?= $bdl['mois'] ?>', true);
    ...
    xhr.onreadystatechange = function () {
      if (xhr.readyState == 4) {
        if (xhr.status == 200) {
          ...
          window.location.href = '?controller=prestataire&action=afficher_bdl&annee=<?= $bdl['annee'] ?>&mois=<?= $bdl['mois'] ?>';
        } else {
